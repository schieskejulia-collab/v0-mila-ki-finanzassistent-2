...
export function milaGreeting(ctx: MilaContext): string {
  const mood = financeMood(ctx.summary)
  return `Hi, schön dass du da bist. ${moodLine[mood]} Dein Gewinn liegt diesen Monat bei ${formatEUR(
    ctx.summary.profit,
...
export function generateMilaReply(message: string, ctx: MilaContext): string {
  ...
  if (/voraus|prognose|vorher|zukunft|monat.*ende|hochrechn/.test(m)) {
    const { projectedProfit, projectedTax } = projectMonthProfit(
      ctx.expenses,
      ctx.incomes,
    )
  ...
  if (/spar|sparen|spartipp|sparpotenzial|günstiger|kosten senken|weniger ausgeben/.test(m)) {
    ...
    const top = tips[0]
    const total = tips.reduce((a, t) => a + t.potential, 0)
  ...
  if (/steuer|rücklage|finanzamt|umsatzsteuer|vat/.test(m)) {
    return `Für deinen aktuellen Gewinn von ${formatEUR(
      summary.profit,
    )} empfehle ich dir eine Steuerrücklage von ${formatEUR(
      summary.taxReserve,
    )} (rund 30 %). Deine Umsatzsteuer-Differenz liegt bei ${formatEUR(
      summary.vatBalance,
  ...
  if (/budget|limit|überschritten|überschreit/.test(m)) {
    const over = budgetStatus.filter((b) => b.level === 'over')
    const warn = budgetStatus.filter((b) => b.level === 'warn')
  ...
  if (/vergleich|letzter monat|vormonat|besser|schlechter/.test(m)) {
    const diff = summary.expenses - prevSummary.expenses
    if (diff < 0)
  ...
  // Default
  const mood = financeMood(summary)
  return `${moodLine[mood]} Dein Gewinn liegt bei ${formatEUR(
    summary.profit,
