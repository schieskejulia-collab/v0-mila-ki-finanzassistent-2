...
export default function JetztPage(){
  ...
  async function load(){
    ...
    const [c,t,u,d]=await Promise.all([
      caseQuery,
    ...
    if(c.error||t.error||u.error||d.error)setError('Mila konnte den Vorgang nicht vollständig laden.')
  ...
  return <main className="min-h-screen bg-[#faf9fc] pb-28 lg:pb-8"><div className="mx-auto w-full max-w-[1220px] px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,.95fr)_minmax(0,1.35fr)]">
      ...
      <div className="min-w-0">{!current?<div className="rounded-2xl border border-dashed bg-white p-10 text-center shadow-sm"><Zap className="mx-auto h-8 w-8 text-violet-500"/><p className="mt-3 font-black">Wähle einen Vorgang.</p></div>:<div className="space-y-4">
        ...
        {resolvedCount>0&&<p className="px-1 text-[11px] font-semibold text-slate-400">{resolvedCount} erledigte Punkte bleiben in der Historie erhalten, werden hier aber nicht mehr als offen angezeigt.</p>}

        <section className="rounded-2xl border border-violet-100 bg-white p-4 shadow-sm lg:p-5"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-violet-500">Übergabe</p><h3 className="mt-1 text-lg font-black">{current.handoff_ready?'Bereit für die fachliche Prüfung':'Noch nicht übergabebereit'}</h3></div>{current.handoff_ready?<FileCheck2 className="h-6 w-6 text-emerald-500"/>:<Clock3 className="h-6 w-6 text-amber-500"/>}</div>{current.handoff_summary&&<pre className="mt-3 whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-xs font-semibold leading-5 text-slate-600">{current.handoff_summary}</pre>}<div className="mt-4 flex flex-col gap-2 sm:flex-row">{!current.handoff_ready&&<button onClick={()=>void prepareHandoff()} disabled={saving} className="flex-1 rounded-xl bg-violet-600 px-3 py-3 text-xs font-black text-white disabled:opacity-50">Übergabe vorbereiten</button>}<button onClick={()=>void finishCase()} disabled={saving||!current.handoff_ready} className="flex-1 rounded-xl bg-emerald-600 px-3 py-3 text-xs font-black text-white disabled:bg-slate-100 disabled:text-slate-400">Vorgang vollständig abschließen</button></div>{!current.handoff_ready&&<p className="mt-3 text-[11px] font-semibold text-slate-500">Benötigt mindestens eine Unterlage und keine offenen Rückfragen/Aufgaben.</p>}</section>
      </div>}</div>
    </section>
  </div></main>
}
