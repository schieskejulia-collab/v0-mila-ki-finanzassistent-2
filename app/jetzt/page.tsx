...
export default function JetztPage(){
  ...
  async function load(requested?:string){
    ...
    let docQuery:any=supabase.from('documents').select('id,case_id,title,file_name,file_url,created_at').order('created_at',{ascending:false}).limit(400)
  ...
  async function setStatus(status:CaseStatus){if(!current||current.status==='done')return;setSaving(true);setError('');const{error:e}=await supabase.from('mila_intake_cases').update({status}).eq('id',current.id);if(e)setError('Status konnte nicht aktualisiert werden.');else setNotice(`Vorgang ist jetzt „${STATUS_LABEL[status]}“.`);setSaving(false);await load(current.id)}
  async function closeTask(task:Task){setSaving(true);const{error:e}=await supabase.from('mila_coordination_tasks').update({status:'done'}).eq('id',task.id);if(e)setError('Arbeitsschritt konnte nicht abgeschlossen werden.');setSaving(false);await load(current?.id)}
  async function answerQuestion(question:Update){
    ...
    else{setAnswerByQuestion(map=>({...map,[question.id]:''}));setNotice('Antwort gespeichert. Die Rückfrage ist geklärt.')}
    setSaving(false);await load(current.id)
  }
  async function prepareHandoff(){if(!current)return;if(currentDocuments.length===0){setError('Noch nicht möglich: Dem Vorgang ist keine Unterlage zugeordnet.');return}if(activeQuestions.length||activeTasks.length){setError('Noch nicht möglich: Erst offene Rückfragen und Arbeitsschritte klären.');return}const summary=[`Anliegen: ${current.subject}`,`Unterlagen: ${currentDocuments.length}`,`Kontakt: ${personLabel(current)}`,current.phone?`Telefon: ${current.phone}`:null,current.email?`E-Mail: ${current.email}`:null,`Zusammenfassung: ${current.summary}`,`Zuständig: ${current.assigned_to||'noch offen'}`].filter(Boolean).join('\n');setSaving(true);const{error:e}=await supabase.from('mila_intake_cases').update({handoff_summary:summary,handoff_ready:true,status:'human_review'}).eq('id',current.id);if(e)setError('Übergabe konnte nicht vorbereitet werden.');else setNotice('Organisatorisch vollständig. Bereit für die fachliche Prüfung.');setSaving(false);await load(current.id)}
  async function finishCase(){if(!current)return;if(!current.handoff_ready||activeQuestions.length||activeTasks.length){setError('Noch nicht abschließen: Übergabe vorbereiten und offene Punkte klären.');return}setSaving(true);const{error:e}=await supabase.from('mila_intake_cases').update({status:'done',completed_at:new Date().toISOString()}).eq('id',current.id);if(e)setError('Vorgang konnte nicht abgeschlossen werden.');else setNotice('Vorgang vollständig abgeschlossen.');setSaving(false);await load(current.id)}
