...
const quickAnswers: Record<string, string[]> = {
  processType: ["Zur Kanzlei", "Nur ablegen", "Später prüfen"],
  businessPurpose: ["Baustelle", "Fahrzeug", "Büro / Betrieb"],
  requestedTime: ["Heute", "Morgen", "Diese Woche"],
}
...
export default function MilaCorePage() {
  ...
  async function runProcess(extraFacts: Record<string, unknown> = facts, textOverride?: string) {
    setLoading(true); setMessage(""); setApproved(false)
  ...
  async function submitAnswer(value?: string) {
    ...
    setMessage("Antwort übernommen ✓ Mila prüft den Vorgang neu …")
  ...
  function clearCaseState() { setCaseId(undefined); setPlan(null); setFacts({}); setAnswered([]); setAnswer(""); setApproved(false); setMessage("") }
  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-6 text-zinc-900"><div className="mx-auto max-w-xl space-y-4">
      ...
      {answered.length > 0 && <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-zinc-200"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-700">Bereits verstanden</p><div className="mt-3 space-y-3">{answered.map((item, i) => <div key={`${item.question}-${i}`} className="rounded-2xl bg-violet-50 p-3"><p className="text-xs text-zinc-500">{item.question}</p><p className="mt-1 font-medium">✓ {item.answer}</p></div>)}</div></section>}

      {question && <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-zinc-200"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-700">Nur noch kurz</p><h2 className="mt-2 text-lg font-semibold">{question.question}</h2><p className="mt-2 text-sm text-zinc-600">{question.reason}</p>{suggestions.length > 0 && <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => submitAnswer(suggestion)} disabled={loading} className="rounded-2xl border border-violet-200 bg-violet-50 px-3 py-3 text-sm font-semibold text-violet-800 disabled:opacity-50">{suggestion}</button>)}</div>}<div className="mt-4 flex gap-2"><input value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") submitAnswer() }} className="min-w-0 flex-1 rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-base outline-none focus:border-zinc-400" placeholder="Oder kurz selbst antworten …"/><button type="button" onClick={() => submitAnswer()} disabled={loading || !answer.trim()} className="rounded-2xl bg-zinc-900 px-4 py-3 font-medium text-white disabled:opacity-50">Senden</button></div></section>}
