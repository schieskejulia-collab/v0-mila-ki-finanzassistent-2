...
export default function NeueBuchungPage(){
 ...
 async function upload(){
  ...
  try{
   ...
   for(const file of files){
    const documentId=crypto.randomUUID(),ext=file.name.split('.').pop()?.toLowerCase()||'bin',path=`${a.user.id}/${clientId}/${targetCaseId}/${documentId}.${ext}`
    ...
    const{error:db}=await supabase.from('documents').insert({id:documentId,user_id:a.user.id,client_id:clientId,case_id:targetCaseId,title:file.name,type:'unterlage',status:'erfasst',file_name:file.name,file_url:path,note:'Original unverändert hochgeladen'})
   ...
   setNotice(`${files.length} Unterlage${files.length===1?'':'n'} erfolgreich mit genau einem Vorgang verbunden.`);setFiles([])
 ...
 <button onClick={()=>void upload()} disabled={saving||!files.length} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3.5 text-sm font-black text-white disabled:bg-slate-200 disabled:text-slate-400">{saving?<><Loader2 className="h-4 w-4 animate-spin"/>Mila verbindet vollständig…</>:<><FolderOpen className="h-4 w-4"/>Stapel in Vorgang übernehmen</>}</button></section></section></div></main>
}
