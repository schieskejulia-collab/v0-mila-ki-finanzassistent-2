...
export default function NeueBuchungPage() {
  ...
  function updateTitle(value: string) {
    setTitle(value)
    if (type === 'expense' && !scannedFile) setCategory(getCategoryLabel(detectCategory(value)))
  }
  ...
  function handleScanSuccess(rawData: any, file: File) {
    ...
    const scannedDocumentDate = String(
      scannedData.documentDate || scannedData.document?.documentDate || ''
    ).trim()
    ...
    const financialDirection = String(scannedData.financialDirection || '').toLowerCase()
    const needsConfirmation = Boolean(scannedData.needsConfirmation || scannedData.needsReview)
    ...
    setType(financialDirection === 'income' ? 'income' : 'expense')
    ...
    setEntryDate(scannedDocumentDate)
    setDueDate(String(scannedData.dueDate || scannedData.due_date || scannedData.document?.dueDate || scannedData.document?.due_date || ''))
    setNote(scannedNote)

    // Scannerwissen ist noch keine steuerliche Kategorie. Inkasso darf als
    // Dokumentart erkannt werden; alles andere startet konservativ ungeprüft.
    setCategory(isInkasso ? INKASSO_LABEL : 'Sonstiges')

    if (!targetExpense && !['expense', 'income'].includes(financialDirection)) {
      setDocumentOnlyMode(true)
      setScanMessage('Mila hat das Dokument gelesen, aber keinen belegten Geldfluss erkannt. Es wird deshalb nur als Dokument abgelegt.')
    } else if (needsConfirmation) {
      setScanMessage('Mila hat die sichtbaren Angaben übernommen. Kontext oder steuerliche Einordnung bleiben ungeprüft.')
    } else {
      setScanMessage('Mila hat die sichtbaren Angaben übernommen. Bitte kurz prüfen und anschließend speichern.')
    }
  ...
  async function speichern() {
    ...
    const separateDocumentOnly = Boolean(
      documentOnlyMode || (scannedFile && receiptMismatch && allowSeparateSave)
    )
    const createsNewFinanceEntry = !receiptMatch && !separateDocumentOnly

    if (createsNewFinanceEntry && !entryDate) {
      alert('Für einen neuen Geldvorgang braucht Mila das tatsächliche Beleg-/Buchungsdatum. Bitte Datum ergänzen.')
      return
    }
    ...
    try {
      ...
      const payload: any = {
        ...
        // Keine pauschale Umsatzsteuer aus dem Scanner erfinden.
        vat: 0,
      }
      ...
      const duplicate = createsNewFinanceEntry && existingItems.find((item: any) => {
      ...
      if (scannedFile) {
        ...
        contentHash = await fileHash(scannedFile)
        const { data: duplicateDocument, error: duplicateDocumentError } = await supabase
          .from('documents')
          .select('id')
        ...
        if (duplicateDocument?.id) {
          throw new Error('Dieser identische Beleg liegt für den ausgewählten Mandanten bereits in der Mappe.')
        }
      ...
      if (partner.trim() && type === 'expense' && !separateDocumentOnly) {
        saveMerchantMemory({
      ...
      let linkedBookingId = receiptMatch?.id ? String(receiptMatch.id) : ''

      if (type === 'expense') {
        if (!receiptMatch && !separateDocumentOnly) {
          payload.id = crypto.randomUUID()
          createdExpenseId = payload.id
          linkedBookingId = payload.id
      ...
      if (scannedFile && uploadedPath && documentId) {
        try {
          await addDocument(createDocument({
            ...
            status: receiptMatch ? 'geprueft' : 'neu',
            documentDate: entryDate || undefined,
            dueDate: dueDate || undefined,
            relatedBookingId: linkedBookingId || undefined,
            ...
            note: receiptMatch
              ...
              : separateDocumentOnly
                ? `${note.trim()}${note.trim() ? ' · ' : ''}Als Dokument abgelegt; kein neuer Geldvorgang erzeugt.`
                : note.trim(),
          }))
          ...
          if (receiptMatch) {
            // Zusätzlich explizit setzen; die Datenbank synchronisiert dies
            // ebenfalls über related_booking_id.
            await markMatchedExpenseAsHavingReceipt(receiptMatch, user.id)
          }
        } catch (documentError) {
          if (createdExpenseId) {
            await deleteExpense({ id: createdExpenseId }).catch(() => undefined)
            createdExpenseId = ''
          }
      ...
      alert(
        receiptMatch
          ...
          : separateDocumentOnly
            ? 'Unterlage wurde als Dokument gespeichert. Es wurde kein neuer Geldvorgang erzeugt.'
            : scannedFile
              ? 'Unterlage und belegter Geldvorgang wurden gespeichert.'
  ...
  return (
    <main className="mx-auto min-h-screen max-w-md space-y-5 p-6 pb-40 text-slate-950">
      ...
      {documentOnlyMode && scannedFile && !receiptMismatch && (
        <section className="rounded-3xl border border-sky-200 bg-sky-50 p-5">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-sky-700">Dokument erkannt</p>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-700">Mila sieht keinen sicher belegten Geldfluss. Beim Speichern entsteht deshalb keine neue Einnahme oder Ausgabe.</p>
        </section>
      )}
      ...
      <section className="border-t border-slate-100 pt-4">
        ...
        <div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-2">
          <button type="button" onClick={() => setType('expense')} disabled={documentOnlyMode} className={type === 'expense' ? 'rounded-2xl bg-white px-4 py-4 text-sm font-black text-slate-950 shadow-sm disabled:opacity-50' : 'rounded-2xl px-4 py-4 text-sm font-black text-slate-500 disabled:opacity-50'}>Ausgabe</button>
          <button type="button" onClick={() => setType('income')} disabled={documentOnlyMode} className={type === 'income' ? 'rounded-2xl bg-white px-4 py-4 text-sm font-black text-violet-700 shadow-sm disabled:opacity-50' : 'rounded-2xl px-4 py-4 text-sm font-black text-slate-500 disabled:opacity-50'}>Einnahme</button>
        </div>
      </section>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Beleg-/Buchungsdatum</label>
          <input type="date" value={entryDate} onChange={(event) => setEntryDate(event.target.value)} className="mt-2 h-14 w-full rounded-2xl border border-violet-100 bg-white px-4 outline-none focus:ring-2 focus:ring-violet-500" />
          {scannedFile && !entryDate && <p className="mt-2 text-xs font-bold text-amber-700">Datum war nicht sicher lesbar. Mila setzt nicht automatisch heute ein.</p>}
        </div>

        {type === 'expense' && !documentOnlyMode && (
      ...
      {type === 'expense' && title && dueDate && !documentOnlyMode && (
      ...
      <button type="button" onClick={speichern} disabled={isSaving || Boolean(receiptMismatch && scannedFile && !allowSeparateSave)} className="w-full rounded-2xl bg-violet-600 py-4 font-black text-white shadow-md disabled:opacity-40">
        {isSaving
          ? 'Speichere ...'
          : receiptMatch
            ? 'Beleg zuordnen & zur Mappe'
            : separateButtonLabel(documentOnlyMode, allowSeparateSave)}
      </button>
...
function separateButtonLabel(documentOnlyMode: boolean, allowSeparateSave: boolean) {
  if (documentOnlyMode) return 'Nur Dokument ablegen'
  if (allowSeparateSave) return 'Nur Dokument separat ablegen'
  return 'Speichern & zur Mappe'
}
