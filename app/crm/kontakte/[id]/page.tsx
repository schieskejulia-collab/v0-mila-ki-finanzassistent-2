...
export default function ContactDetailPage() {
  ...
  async function addProject() {
    ...
    const { error: insertError } = await supabase.from('crm_projects').insert({
  ...
  async function addNote() {
    ...
    const { error: insertError } = await supabase.from('crm_notes').insert({
      user_id: userData.user.id,
      contact_id: contactId,
      body: noteBody.trim(),
      note_type: noteType,
  ...
  async function addEvent() {
    ...
    const { error: insertError } = await supabase.from('crm_calendar_events').insert({
      user_id: userData.user.id,
      contact_id: contactId,
  ...
  if (!contact) {
    return <main className="min-h-screen p-6 font-bold">{error || 'Kontakt wird geladen...'}</main>
  }
  ...
  return (
    <main className="min-h-screen space-y-5 bg-[#fbf9ff] p-4 pb-36 text-slate-950">
      <Link href="/crm" className="font-black text-violet-600">Zurueck zum CRM</Link>
      ...
      <section className="rounded-[2rem] bg-white p-5 shadow-sm">
        ...
        {contact.email && <p className="mt-4 text-sm font-semibold">E-Mail: {contact.email}</p>}
        {contact.phone && <p className="mt-1 text-sm font-semibold">Telefon: {contact.phone}</p>}
      ...
      <section className="space-y-3 rounded-[2rem] bg-white p-5 shadow-sm">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">Projekt hinzufuegen</p>
        ...
        <select className="w-full rounded-2xl border p-4" value={projectStatus} onChange={(e) => setProjectStatus(e.target.value)}>
        ...
        {projects.map((project) => <div key={project.id} className="rounded-2xl bg-violet-50 p-4"><p className="font-black">{project.title}</p><p className="text-sm text-slate-600">{project.description}</p><p className="mt-2 text-xs font-black text-violet-700">{projectStatuses[project.status as keyof typeof projectStatuses] || project.status}</p></div>)}
      </section>

      <section className="space-y-3 rounded-[2rem] bg-white p-5 shadow-sm">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">Notiz oder Kommunikation</p>
        <select className="w-full rounded-2xl border p-4" value={noteType} onChange={(e) => setNoteType(e.target.value)}>
          {Object.entries(noteTypes).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
        </select>
        ...
        {notes.map((note) => <div key={note.id} className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{noteTypes[note.note_type as keyof typeof noteTypes] || note.note_type} - {new Date(note.created_at).toLocaleDateString('de-DE')}</p><p className="mt-2 whitespace-pre-line text-sm font-semibold text-slate-700">{note.body}</p></div>)}
      </section>
      ...
      <section className="space-y-3 rounded-[2rem] bg-white p-5 shadow-sm">
        ...
        <textarea className="min-h-20 w-full rounded-2xl border p-4" placeholder="Terminnotiz oder Videolink" value={eventNotes} onChange={(e) => setEventNotes(e.target.value)} />
        ...
        {events.map((event) => <div key={event.id} className="rounded-2xl bg-amber-50 p-4"><p className="font-black">{event.title}</p><p className="text-sm font-bold text-amber-700">{new Date(event.starts_at).toLocaleString('de-DE')}</p>{event.notes && <p className="mt-1 text-sm text-slate-600">{event.notes}</p>}</div>)}
      </section>
